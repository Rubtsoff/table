...
const TableRow: FC<ITableRow> = ({user}) => {
  ...
  return (
    <div className="row">
      ...
      <TableCell>
        {readOnly
          ? <button className="button" onClick={onEdit} disabled={isLoading}>Edit</button>
          : <button className="button button--success" onClick={onSave} disabled={isLoading}>Save</button>
        }
      </TableCell>
      <TableCell>
        <button className="button button--error" onClick={onDelete} disabled={isLoading}>Delete</button>
      </TableCell>
    </div>
  );
};
